...
@OPTIMIZER_BUILDERS.register_module()
class LearningRateDecayOptimizerConstructor_custom(DefaultOptimizerConstructor):
    """Different learning rates are set for different layers of backbone.
    Note: Currently, this optimizer constructor is built for ConvNeXt
    and BEiT.
    """
            ...
            if 'layer_wise' in decay_type:
                if 'ConvNeXt' in module.backbone.__class__.__name__:
                    if not is_custom:                 
                        ...
                        name, self.paramwise_cfg.get('num_layers'))
                    logger.info(f'set param {name} as id {layer_id}')
                elif 'BEiT' in module.backbone.__class__.__name__ or 'ViT' in module.backbone.__class__.__name__ or 'VisionTransformer' in module.backbone.__class__.__name__:
                    if not is_custom:                 
                        layer_id = get_layer_id_for_vit(name, num_layers)
                    logger.info(f'set param {name} as id {layer_id}')
                else:
