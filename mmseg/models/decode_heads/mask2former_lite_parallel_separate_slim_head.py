...
from mmcv.cnn.utils.weight_init import (constant_init, trunc_normal_,
                                        trunc_normal_init)
from mmcv.cnn import Conv2d, ConvModule, build_plugin_layer, caffe2_xavier_init
...
class Mask2Former_lite_parallel_separate_slim_Head(BaseDecodeHead):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 **kwargs):
        super(Mask2Former_lite_parallel_separate_slim_Head, self).__init__(
        ...
        self.norm = nn.SyncBatchNorm(self.in_channels)    
        #self.act = nn.ReLU(inplace=True)      
        self.pixel_proj1 = ConvModule(
            self.in_channels,
            self.decoder_embed_dims * self.num_transformer_feat_level,
            3,
            padding=1,
        ...
        self.memory_proj = ConvModule(
            self.decoder_embed_dims * self.num_transformer_feat_level,
            self.decoder_embed_dims * self.num_transformer_feat_level,
            3,
            padding=1,
            groups=self.num_transformer_feat_level,
            conv_cfg=self.conv_cfg,
            norm_cfg=dict(type='SyncBN', requires_grad=True),
            act_cfg=None)
        
        #self.memory_proj = Conv2d(
        #    self.decoder_embed_dims, self.decoder_embed_dims, kernel_size=1, stride=1, padding=0)
        self.pixel_proj2 = ConvModule(
            self.decoder_embed_dims * self.num_transformer_feat_level,
    ...
    def forward_head(self, decoder_out, mask_feature, attn_mask_target_size, i):
        ...
        nums = len(self.attn_thre)
        ratio = self.num_transformer_decoder_layers // nums
        attn_mask = attn_mask.sigmoid() < self.attn_thre[(i//ratio) % nums]
    ...
    def forward(self, feats, img_metas):
        ...
        x = resize(x, size=[h*2, w*2], mode='bilinear', align_corners=self.align_corners)
        x = self.pixel_proj1(self.norm(x))
        multi_scale_memorys = torch.split(self.memory_proj(x), self.decoder_embed_dims, dim=1)
        x = resize(x, size=[h*4, w*4], mode='bilinear', align_corners=self.align_corners)
        mask_features = self.mask_feature(self.pixel_proj2(x))          
        decoder_inputs = []
        ...
        cls_pred, mask_pred, attn_mask = self.forward_head(
            query_feat, mask_features, multi_scale_memorys[0].shape[-2:], 0)
        ...
        for i in range(self.num_transformer_decoder_layers):
            ...
            cls_pred, mask_pred, attn_mask = self.forward_head(
                query_feat, mask_features, multi_scale_memorys[
                    (i + 1) % self.num_transformer_feat_level].shape[-2:], i+1)
